...
@section('content')
    @php
        // Inisialisasi variabel untuk memastikan tidak ada error 'undefined variable'
        // jika variabel ini tidak di-pass dari controller.
        $antrian = $antrian ?? null;
        $dokterList = $dokterList ?? collect();
        $riwayatList = $riwayatList ?? collect();
        $allDokter = $allDokter ?? collect();
    @endphp
    {{-- Referensi untuk ikon Bootstrap --}}
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css">

    {{--
    Menggunakan Alpine.js untuk state management sederhana di front-end.
    'showBookingForm' akan mengontrol visibilitas modal untuk mengambil antrian.
    --}}
    <div x-data="{ showBookingForm: false }" class="bg-gradient-to-br from-blue-50 to-indigo-100 min-h-screen">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-2xl text-white p-8 mb-8 shadow-lg">
                ...
                <p class="text-blue-100">Sistem Antrian Online kami siap melayani Anda. Silakan pilih salah satu menu di
                    bawah untuk memulai.</p>
            </div>
            ...
            {{-- Status Antrian Saat Ini (Hanya tampil jika ada antrian aktif) --}}
            @if ($antrian)
                <div class="bg-white rounded-2xl shadow-xl p-6 mb-8 border-l-4 border-green-500">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <div class="text-center">
                            <div class="text-lg font-semibold text-gray-900">{{ $antrian->gelar }} {{ $antrian->nama_dokter }}
                            </div>
                            <div class="text-sm text-gray-600">Dokter</div>
                        </div>
                        <div class="text-center">
                            <div class="text-lg font-semibold text-gray-900">
                                {{ \Carbon\Carbon::parse($antrian->jam_kunjungan)->format('H:i') }}</div>
                            <div class="text-sm text-gray-600">
                                {{ \Carbon\Carbon::parse($antrian->tanggal_kunjungan)->translatedFormat('d M Y') }}</div>
                        </div>
                        <div class="text-center">
                    ...
                    <div class="mt-6 text-center">
                        <form action="{{ route('pengunjung.antrian.batalkan', $antrian->id) }}" method="POST"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div class="bg-white rounded-2xl shadow-xl p-6 hover:shadow-2xl transition-shadow flex flex-col">
                    ...
                    <a href="{{ route('pengunjung.jadwal') }}"
                        class="w-full mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center justify-center">
                        Lihat Semua Jadwal <i class="bi bi-arrow-right-short ml-2"></i>
                    </a>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    {{-- ... (Kartu Jadwal, Status, Riwayat tetap sama) ... --}}
                    <!-- Ambil Nomor Antrian -->
                    <div class="bg-white rounded-2xl shadow-xl p-6 hover:shadow-2xl transition-shadow flex flex-col">
                        <div class="flex items-center mb-4">
                ...
                <div class="bg-white rounded-2xl shadow-xl p-6 hover:shadow-2xl transition-shadow flex flex-col">
                    ...
                    <div class="text-center py-8 flex-grow">
                        @if ($antrian)
                            ...
                            <div class="text-gray-600">Nomor antrian Anda <span
                                    class="font-semibold">{{$antrian->status_antrian}}</span></div>
                        @else
                    ...
                    <a href="{{ route('pengunjung.status') }}"
                ...
                <div class="bg-white rounded-2xl shadow-xl p-6 hover:shadow-2xl transition-shadow flex flex-col">
                            ...
                            <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                                <div>
                                    <div class="font-medium text-gray-900">
                                        {{ \Carbon\Carbon::parse($riwayat->tanggal_kunjungan)->translatedFormat('d F Y') }}
                                    </div>
                    ...
                    <a href="{{ route('pengunjung.riwayat') }}"
                        class="w-full mt-4 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 flex items-center justify-center">
                        Lihat Semua Riwayat <i class="bi bi-arrow-right-short ml-2"></i>
                    </a>
                </div>
            </div>

        <!-- Modal Form Booking (Logika Baru) -->
        <div x-show="showBookingForm" @keydown.escape.window="showBookingForm = false" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" style="display: none;">
            <div x-show="showBookingForm" @click.outside="showBookingForm = false" x-transition class="bg-white rounded-2xl max-w-md w-full p-6 max-h-[90vh] overflow-y-auto">
                <div class="flex justify-between items-center mb-6">
                    <h3 class="text-xl font-bold text-gray-900">Ambil Nomor Antrian</h3>
                    <button @click="resetForm(); showBookingForm = false" class="text-gray-500 hover:text-gray-700 text-2xl">&times;</button>
                </div>
                ...
                <form method="POST" action="{{ route('pengunjung.ambil-antrian') }}">
                    ...
                    <div class="space-y-4">
                        ...
                        <!-- 2. Pilih Jadwal & Dokter (Dinamis) -->
                        <div>
                            <label for="dokter_id" class="block text-sm font-medium text-gray-700 mb-2">Pilih Jadwal & Dokter</label>
                            <div x-show="isLoading" class="text-sm text-gray-500 p-2">
                            ...
                            <select name="dokter_id" x-show="!isLoading && schedules.length > 0" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" required>
                                ...
                                <template x-for="schedule in schedules" :key="schedule.dokter_id + schedule.jam_mulai">
                                    <option :value="schedule.dokter_id" x-text="formatSchedule(schedule)"></option>
                                </template>
                            </select>
                            <div x-show="!isLoading && selectedPoli && schedules.length === 0" class="text-sm text-red-500 p-2 bg-red-50 rounded-lg">
                                Tidak ada jadwal tersedia untuk poli ini hari ini.
                            </div>
                        </div>
                    ...
                    <div class="mt-6 flex gap-3">
                        ...
                        <button type="button" @click="resetForm(); showBookingForm = false" class="flex-1 px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50">
                            Batal
                        </button>
                    </div>
                </form>
